...
from keyboards.default.admins_btn import cancel_brn, remove
from loader import bot
from aiogram.types import Message, CallbackQuery
from data.config import ADMINS
from aiogram import Dispatcher
from keyboards.inline.admin_btn import admin_btn
...
async def start_admin_handler(message: Message):
    ...
    if user_id in ADMINS:
        ...
        await message.answer(f"Siz admin paneldasiz:\n\n"
                             f"Bot azolari soni: {users}", reply_markup=btn)


async def send_message_to_users_callback(call: CallbackQuery):
    await call.message.delete()
    btn = await cancel_brn()
    await call.message.answer("Xabaringizni yolang...", reply_markup=btn)
...
async def get_admin_message_state(message: Message, state: FSMContext):
    ...
    await message.answer("Xabar yollash boshlandi", reply_markup=remove)
    await state.finish()
